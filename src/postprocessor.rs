...
impl ImageExt for Vec<Vec<u8>> {
    fn to_grayscale(&self) -> OutputImage {
        OutputImage::new_grayscale(self.clone())
    }
    ...
    fn scale(&mut self, factor: usize) {
        ...
        let width = self.first().map_or(0, |row| row.len());
        let height = self.len();
...
impl OutputImage {
    ...
    pub fn new_grayscale(bitmap: Vec<Vec<u8>>) -> Self {
    ...
    fn apply_filter<F: Fn(&mut RgbImage, f32)>(&mut self, f: F, strength: f32) {
        self.convert_to_rgb();
        match self {
            Self::Rgb(img) => {
                f(img, strength);
