...
pub fn sketch(image: &mut RgbImage, strength: f32, verbose: bool) {
...
pub fn space(image: &mut RgbImage, strength: f32, verbose: bool) {
    // Step 1: Introduce random noise to simulate stars
    if verbose {
        println!("Step 1... Introducing random noise to simulate stars");
    }
    ...
    if verbose {
        println!("Step 2... Applying subtle blur and sepia");
    }
    ...
    if verbose {
        println!("Step 3... Applying highpass filter to enhance edges");
    }
    highpass_filter(image, &blurred, strength);

    // Step 4: Unsharp masking to highlight glyphs
    if verbose {
        println!("Step 4... Applying unsharp masking to highlight glyphs");
    }
    unsharp_mask(image, &blurred, strength);
}

pub fn granite(image: &mut RgbImage, strength: f32, verbose: bool) {
    ...
    image
        ...
        .for_each(|(px, ox)| {
            let origin_lum = (f32::from(ox[0]) + f32::from(ox[1]) + f32::from(ox[2])) / 3.0 / 255.0;
            let lum = (1.0 - origin_lum) * 0.5 * strength * 255.0;

            let r = (f32::from(px[0]) + lum).clamp(0.0, 255.0);
            let g = (f32::from(px[1]) + lum).clamp(0.0, 255.0);
            let b = (f32::from(px[2]) + lum).clamp(0.0, 255.0);
