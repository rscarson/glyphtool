...
use super::{
    phonambulator::{PhonambulationSource, Phonambulator},
    preprocessor::preprocess_word,
    token::Token,
};
use crate::error::EtroisResult;

/// Trait for making use of the lexer
pub trait TokensExt {
    ...
    ///
    /// # Errors
    /// Will return an error if a database operation fails, or if the phonambulation fails
    fn phonambulate<S: PhonambulationSource>(
        &mut self,
        db_path: Option<&str>,
        source: S,
    ) -> EtroisResult<()>;
...
impl TokensExt for Vec<Token> {
    fn from_string(input: &str) -> Self {
        ...
        for token in Token::lexer(&text) {
            match token {
                ...
                Err(()) => tokens.push(Token::WordBoundary), // Treat all unknown symbols as word boundaries
            }
        }
    ...
    fn phonambulate<S: PhonambulationSource>(
        ...
        let mut phonambulator = Phonambulator::new(db_path, source)?;
        for token in self {
...
mod test {
    ...
    fn test_reduce() {
        ...
        let mut tokens = Vec::<Token>::from_string(text);
        tokens.reduce();
